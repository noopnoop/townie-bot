...
import { Listings, NormalInteraction, NormalMember, PlayerDB } from '../types';
...
export async function executeLeaveGame (interaction : CommandInteraction & NormalInteraction, playerDb : PlayerDB, listings : Listings) {
  ...
  const guildDb = listings.get(currentGuild);
  ...
  listings.set(currentGuild, guildDb);
